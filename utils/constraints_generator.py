...
def constraints_generator(max_dom, num_vars, density):
    cons_map = [[[[0 for _ in range(max_dom)] for _ in range(max_dom)] for _ in range(num_vars)] for _ in range(num_vars)]
    for i in range(num_vars):
        rel_self = cons_map[i][i]
        for j in range(max_dom):
            rel_self[j][j] = 1
        for j in range(i+1, num_vars):
            rel_ij = cons_map[i][j]
            rel_ji = cons_map[j][i]
            for ii in range(max_dom):
                for jj in range(max_dom):
...
print(bm_name, "| Duration =", time.time() - ticks)

f = open('../rand_benchmark/' + bm_name, 'wb')
